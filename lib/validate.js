...
import { formatError, createErrorResponse } from './errors.js';
...
function validate(schema, options = {}) {
  ...
  return (req, res, next) => {
    try {
      ...
      const targetKey = sourcePath[sourcePath.length - 1];
      const currentTarget = target[targetKey] || {};

      if (stripUnknown) {
        for (const key of Object.keys(currentTarget)) {
          delete currentTarget[key];
        }
        Object.assign(currentTarget, validatedData);
      } else {
        Object.assign(currentTarget, validatedData);
      }

      req.validated = req.validated || {};
      req.validated[sourcePath.join('.')] = validatedData;
